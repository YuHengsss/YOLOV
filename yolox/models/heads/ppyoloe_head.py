...
class PPYOLOEHead(nn.Module):
    ...
    def add_param_group(self, param_groups, base_lr, base_wd, need_clip, clip_norm):
        for i in range(len(self.in_channels)):
            ...
            if self.obj_preds[i].weight.requires_grad:
                param_group_conv_weight3 = {'params': [self.pred_reg[i].weight]}
                param_group_conv_weight3['lr'] = base_lr * 1.0
                param_group_conv_weight3['base_lr'] = base_lr * 1.0
                param_group_conv_weight3['weight_decay'] = base_wd
                param_group_conv_weight3['need_clip'] = need_clip
                param_group_conv_weight3['clip_norm'] = clip_norm
                param_groups.append(param_group_conv_weight3)
            if self.obj_preds[i].bias.requires_grad:
                param_group_conv_bias3 = {'params': [self.obj_preds[i].bias]}
                param_group_conv_bias3['lr'] = base_lr * 1.0
                param_group_conv_bias3['base_lr'] = base_lr * 1.0
                param_group_conv_bias3['weight_decay'] = base_wd
                param_group_conv_bias3['need_clip'] = need_clip
                param_group_conv_bias3['clip_norm'] = clip_norm
                param_groups.append(param_group_conv_bias3)
    ...
    def forward(self, feats, targets=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
                zip(self.stem_cls, self.stem_reg, self.strides, feats)
        ):
            x = self.stems[k](x)
            avg_feat = F.adaptive_avg_pool2d(x, (1, 1))
            cls_logit = self.pred_cls[k](self.stem_cls[k](x, avg_feat) +
                                         x)
            reg_distri = self.pred_reg[k](self.stem_reg[k](x, avg_feat))
            obj_output = self.obj_preds[k](self.stem_reg[k](x, avg_feat))
            if self.training:
        ...
        if self.training:
            ...
            output = self.decode_outputs(outputs, dtype=feats[0].type())
            return output
