...
from yolox.models.post_process import postprocess as postprocess
...
class PPYOLOEHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.trans = MSA_yolov(dim=self.width,out_dim=4*self.width,num_heads=heads,attn_drop=drop,scale=scale)
    ...
    def forward(self, feats, targets=None, imgs=None):
        ...
        features_cls,features_reg,cls_scores,fg_scores = self.find_feature_score(cls_feat_flatten, pred_idx,reg_feat_flatten,imgs,pred_result)#[b*50,5,128]
        ...
        if self.use_score:
            trans_cls = self.trans(features_cls,features_reg,cls_scores,fg_scores,sim_thresh=self.sim_thresh)#imgs,pred_result
        else:
            trans_cls = self.trans(features_cls, features_reg,)
