...
class FCOSHead(torch.nn.Module):
        ...
        for i in range(len(fpn_stride)):
            self.cls_convs2.append(
                nn.Sequential(
                    *[
                        Conv(
                            in_channels=int(self.in_channel),
                            out_channels=int(self.in_channel),
                            ksize=3,
                            stride=1,
                        ),
                        Conv(
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (fpn_feat, fpn_stride) in enumerate(zip(xin, self.fpn_stride)):
            ...
            n_cls_features = self.cls_convs2[k](fpn_feat)

            cls_features.append(n_cls_features)
        ...
        if self.test_freaze: return decode_res
