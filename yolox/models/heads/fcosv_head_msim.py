...
from yolox.models.post_process import postprocess_fcos as postprocess
...
class FCOSHead(torch.nn.Module):
    def __init__(self,
        ...
        self.linear0 = nn.Linear(2 * self.width,
                                 2 * self.width)  # Mlp(in_features=256,hidden_features=256)#nn.Linear(256,256)
        self.linear1 = nn.Linear(4 * self.width, 31)  # Mlp(in_features=512,hidden_features=self.num_classes+1)
        ...
        self.n_anchors = 1
        self.use_l1 = False
        self.trans = MSA_yolov(dim=self.width, out_dim=4 * self.width, num_heads=4, attn_drop=0)
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        if self.training:
            self.Afternum = 30
            self.simN = int(xin[0].shape[0] / 2)
        else:
            self.Afternum = 75
            self.simN = int(xin[0].shape[0])

        for k, (fpn_feat, fpn_stride) in enumerate(zip(xin, self.fpn_stride)):
            ...
            cls_feat2 = self.cls_convs2(fpn_feat)
            cls_features.append(cls_feat2)
            reg_features.append(subnet_blob_reg)

            cls_logits = self.cls_pred(subnet_blob_cls)  # 通道数变成类别数
            reg_output = self.reg_pred(subnet_blob_reg)  # 通道数变成4
            centerness = self.ctn_pred(subnet_blob_reg)
            output_decode = torch.cat(
                [reg_output, centerness.sigmoid(), cls_logits.sigmoid()], 1
            )
            if self.training:
                output = torch.cat([reg_output, centerness, cls_logits], 1)
                output, grid = self.get_output_and_grid(
                    output, k, fpn_stride, xin[0].type()
                )
    ...
    def get_losses(
        ...
        debug_ref_cls = torch.max(refined_cls.view(-1, self.num_classes)[ref_masks],dim=-1)
        debug_ref_tar = torch.max(ref_targets[ref_masks],dim=-1)
        if self.use_l1:
            loss_l1 = (
    ...
    def postprocess_single_img(self,prediction, num_classes,conf_thre=0.001, nms_thre=0.5,class_agnostic = True):
